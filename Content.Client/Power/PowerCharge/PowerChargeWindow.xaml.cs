...
public sealed partial class PowerChargeWindow : FancyWindow
{
    ...
    public void UpdateState(PowerChargeState state)
    {
        ...
        var showStationStatus = state.HasStationStatus;
        ...
        {
            var roundedMass = Math.Round(state.StationMass, 1);
            MassValue.Text = Loc.GetString("gravity-generator-window-mass-value", ("mass", roundedMass));

            var ftlKey = state.StationFtlLocked ? "gravity-generator-examine-ftl-locked" : "gravity-generator-examine-ftl-free";
            FtlValue.Text = Loc.GetString(ftlKey);
            FtlValue.SetOnlyStyleClass(state.StationFtlLocked ? "Danger" : "Good");
        }
        else
        {
            MassValue.Text = "—";
